...
def add_ely_account(username, email, password):
    ...
    auth_url = "https://ely.by/account/session"
    session = requests.Session()
    try:
        auth_data = {"email": email, "password": password, "remember": "true"}
        response = session.post(auth_url, data=auth_data)
        if response.status_code == 200:
            accounts = load_accounts()
            account_id = max([acc["id"] for acc in accounts], default=0) + 1
            session_data = {"cookies": dict(session.cookies), "headers": dict(session.headers)}
            account = {
                "id": account_id,
                "username": username,
                "type": "ely",
...
def manage_accounts_scrollable():
    ...
    if not accounts:
        ...
        if choice == '1':
            ...
            password = getpass.getpass("Введите пароль от Ely.by: ")
...
def launch_minecraft_thread(version, java_path, java_args_str, username, minecraft_dir, account_type):
    try:
        options = {'username': username, 'uuid': '', 'token': ''}
        if account_type == 'ely':
            print(f"{COLOR_YELLOW}Предупреждение: Авторизация Ely.by требует дополнительной реализации{COLOR_RESET}")
...
def launch_minecraft(version=None):
    ...
    thread = threading.Thread(
        ...
        args=(version, java_path, java_args_str, username, minecraft_dir, account.get('type')),
        daemon=True
    )
    thread.start()
